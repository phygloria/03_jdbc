...
public class EmployeeService {
    ...
    public EmployeeDTO empModify(EmployeeDTO emp) throws Exception {
        EmployeeDTO modifyEmp = employeeRepository.empFindById(emp.getEmpId());

        int result = employeeRepository.empModify(emp);
        if(result < 0){
