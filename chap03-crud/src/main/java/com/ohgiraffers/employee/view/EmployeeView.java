...
public class EmployeeView {
    ...
    public static void empInsert(){
        ...
        System.out.println("사원의 주민번호을 입력하세요 :");
        emp.setEmpNo(sc.nextLine());
        System.out.println("사원의 직급을 입력하세요 : 대표, 부사장, 부장, 차장, 과장, 대리, 사원");
        emp.setJobCode(sc.nextLine());
        System.out.println("사원의 급여를 입력하세요 : S1, S2, S3, S4, S5, S6");
    ...
    public static void empUpdate(){
        ...
        try {
            EmployeeDTO modifyEmp = employeeService.empModify(emp);
...
// static은 메모리에 바로 올려주는 건데 얘를 호출해서 쓰려면 클래스를 스태틱으로 올려주거나 아래 메소드에서 스태틱올려주면된다.